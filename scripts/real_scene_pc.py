...
if __name__ == "__main__":
    ...
    for json_file in json_files:
        ...
        if len(grasps) > 0:
            ...
            # print(collision_free_grasps)
            all_r = []
            all_t = []
            transformed_grasp = tra.inverse_matrix(T_center) @ collision_free_grasps
            for good_grasp in transformed_grasp:
                # print(tra.inverse_matrix(T_center) @ grasp)
                t = good_grasp[:3, 3]
                q = R.from_matrix(good_grasp[:3, :3]).as_quat()

                rot = R.from_matrix(good_grasp[:3, :3])
            ...
            # print(all_t[0])
            # print(all_r[0])
            np.save("/home/crslab/cehao/data/grasp/tran.npy", np.array(all_t))
            np.save("/home/crslab/cehao/data/grasp/rot.npy", np.array(all_r))
